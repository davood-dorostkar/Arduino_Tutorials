...
void turnLeft() {}
void brake() {}
void situation(uint16_t driveAmplitude,uint16_t turnAmplitude) {
  driveAmplitude=map(driveAmplitude,0,1024,0,100);
  turnAmplitude=map(turnAmplitude,0,1024,0,100);
  if (abs(driveAmplitude-50)<10 || abs(turnAmplitude-50)<10)
    brake();  
  else if (driveAmplitude>60){
